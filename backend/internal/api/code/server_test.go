...
func TestEmailCodeEndpoint(t *testing.T) {
	...
	latest, err := queries.CreateMessage(ctx, gen.CreateMessageParams{
		...
		FromAddr:   "Service <noreply@mail.example.com>",
		ToAddr:     "User <user@example.com>",
		Subject:    sql.NullString{String: "新验证码", Valid: true},
		ReceivedAt: time.Now().Unix(),
		TextBody:   sql.NullString{String: "您的验证码是 222222", Valid: true},
		Size:       10,
	})
	if err != nil {
	...
	req := httptest.NewRequest(http.MethodPost, "/email", bytes.NewBufferString(`{"recipient":"user@example.com","sender_suffix":"example.com"}`))
...
func TestEmailCodeEndpointMarkReadFalse(t *testing.T) {
	...
	msg, err := queries.CreateMessage(ctx, gen.CreateMessageParams{
		...
		ReceivedAt: time.Now().Unix(),
		TextBody:   sql.NullString{String: "OTP: 333333", Valid: true},
		Size:       10,
	})
	if err != nil {
	...
	req := httptest.NewRequest(http.MethodPost, "/email", bytes.NewBufferString(`{"recipient":"user@example.com","sender_suffix":"example.com","mark_read":false}`))
	req.Header.Set("Authorization", "Bearer "+apiKey)
	rr := httptest.NewRecorder()
