...
export default function SystemConfig() {
  ...
  const canSave = !!current && !isBusy && (current.editable.ingest_token || current.editable.ingest_secret || current.editable.session_secret || current.editable.admin_username || current.editable.admin_password);
      ...
      <div className="max-w-5xl">
        <Panel className="p-5" accent>
          <div className="space-y-5">
            ...
            <Field label="INGEST_SECRET" hint={current?.editable.ingest_secret ? "Worker 签名校验密钥，留空则不修改。" : "当前由环境变量 INGEST_SECRET 接管，不能在网页修改。"}>
              <SecretInput
                value={ingestSecret}
                onChange={setIngestSecret}
                disabled={!current?.editable.ingest_secret || isBusy}
                placeholder={current?.ingest_secret_set ? "留空则不修改" : "请输入新密钥"}
                onGenerate={() => setSaved(false)}
              />
            </Field>

            <Field label="SESSION_SECRET" hint={current?.editable.session_secret ? "后台登录会话签名密钥，留空则不修改。" : "当前由环境变量 SESSION_SECRET 接管，不能在网页修改。"}>
              <SecretInput
                value={sessionSecret}
  ...
  return (
    <div className="flex flex-col gap-2 lg:flex-row">
      ...
      <div className="grid grid-cols-2 gap-2 lg:flex lg:shrink-0">
        <Button type="button" variant="secondary" className="whitespace-nowrap px-3" disabled={disabled} onClick={() => setShow((current) => !current)}>
          {show ? "隐藏" : "显示"}
        </Button>
        <Button
          type="button"
